# Debug Draw Utility for V2
# A reusable debug visualization tool for drawing lines, spheres, rays, and shapes

extends Node3D
class_name DebugDrawV2

static var instance: DebugDrawV2 = null
...
func _draw_sphere(center: Vector3, radius: float, color: Color, segments: int) -> void:
	_draw_circle(center, radius, color, segments, Vector3.FORWARD)
	_draw_circle(center, radius, color, segments, Vector3.UP)
	_draw_circle(center, radius, color, segments, Vector3.RIGHT)
