...
void main() {
    ...
    vec2 raw_data[8];
    for(int i=0; i<8; i++) {
        raw_data[i] = get_voxel_data(corners[i]);
    }

    // 2. PROCESS TERRAIN PASS (ID 1)
    {
        float densities[8];
        int cubeIndex = 0;
        for(int i=0; i<8; i++) {
            float d = raw_data[i].x;
            float m = raw_data[i].y;
            if (m > 1.5) d = 0.0; // Treat Water as Surface
            densities[i] = d;
            if (d < ISO_LEVEL) cubeIndex |= (1 << i);
        }
        ...
        if (edgeTable[cubeIndex] != 0) {
            ...
            for (int i = 0; triTable[cubeIndex * 16 + i] != -1; i += 3) {
                uint idx = atomicAdd(counter.count_terrain, 1);
                uint start_ptr = idx * 18; 
                ...
                vec3 n1 = get_normal_for_pass(v1, 1);
                vec3 n2 = get_normal_for_pass(v2, 1);
                vec3 n3 = get_normal_for_pass(v3, 1);
    ...
    // 3. PROCESS WATER PASS (ID 2)
    {
        ...
        for(int i=0; i<8; i++) {
            float d = raw_data[i].x;
            float m = raw_data[i].y;
            if (m > 0.5 && m < 1.5) d = 0.0; // Treat Terrain as Surface
            densities[i] = d;
            if (d < ISO_LEVEL) cubeIndex |= (1 << i);
        }
        ...
        if (edgeTable[cubeIndex] != 0) {
            ...
            for (int i = 0; triTable[cubeIndex * 16 + i] != -1; i += 3) {
                uint idx = atomicAdd(counter.count_water, 1);
                uint start_ptr = idx * 18; 
                ...
                vec3 n1 = get_normal_for_pass(v1, 2);
                vec3 n2 = get_normal_for_pass(v2, 2);
                vec3 n3 = get_normal_for_pass(v3, 2);
